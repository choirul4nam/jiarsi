...
?><div class="sc_donations_info_inner<?php
    if ( !empty($subtitle_info) || !empty($show_title)) echo esc_attr(' show_title_wrap');
    if ( !empty($show_featured) ) echo esc_attr(' show_featured');
    if ( !empty($icon_info) ) echo esc_attr(' show_icon');
?>"><?php
    ...
    // Icon Info
    if ( !empty($icon_info) ) {
        ?><div class="sc_donations_icon_info"><span class="icon-<?php echo esc_attr($icon_info); ?>"></span></div><?php
    }

    if ( !empty($subtitle_info) || !empty($show_title)) { ?>
        <div class="sc_donations_info_wrap_title"><?php
    }

        // Title
        if ( !empty($show_title) ) {
            the_title( sprintf( '<h6 class="sc_donations_title entry-title"><a href="%s" rel="bookmark">', esc_url(get_permalink()) ), '</a></h6>' );
        }

        // Subitle Info
        if ( !empty($subtitle_info) ) {
            ?><div class="sc_donations_subtitle_info"><?php echo esc_html($subtitle_info); ?></div><?php
        }

    if ( !empty($icon_info) || !empty($show_title)) {
        ?></div><?php
    }
