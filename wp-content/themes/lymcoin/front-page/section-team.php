<div class="front_page_section front_page_section_team<?php
			$lymcoin_scheme = lymcoin_get_theme_option('front_page_team_scheme');
			if (!lymcoin_is_inherit($lymcoin_scheme)) echo ' scheme_'.esc_attr($lymcoin_scheme);
			echo ' front_page_section_paddings_'.esc_attr(lymcoin_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$lymcoin_css = '';
		$lymcoin_bg_image = lymcoin_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$lymcoin_anchor_icon = lymcoin_get_theme_option('front_page_team_anchor_icon');	
	$lymcoin_anchor_text = lymcoin_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (lymcoin_get_theme_option('front_page_team_fullheight'))
				echo ' lymcoin-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$lymcoin_css = '';
			$lymcoin_bg_mask = lymcoin_get_theme_option('front_page_team_bg_mask');
			$lymcoin_bg_color = lymcoin_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$lymcoin_caption = lymcoin_get_theme_option('front_page_team_caption');
			if (!empty($lymcoin_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($lymcoin_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($lymcoin_caption, 'lymcoin_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$lymcoin_description = lymcoin_get_theme_option('front_page_team_description');
			if (!empty($lymcoin_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($lymcoin_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($lymcoin_description), 'lymcoin_kses_content'); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!lymcoin_exists_trx_addons())
						...
						lymcoin_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
