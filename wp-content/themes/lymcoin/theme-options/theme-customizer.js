/* global lymcoin_color_schemes, lymcoin_dependencies, Color */
...
( function( api ) {
	...
	for (var i in lymcoin_color_schemes) {
		cssTemplate[i] = wp.template( 'lymcoin-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'lymcoin-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+lymcoin_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(lymcoin_customizer_vars['msg_reset_confirm'])) {
		...
		if (lymcoin_customizer_vars && lymcoin_customizer_vars['theme_name_suffix'])
			jQuery('#customize-info .site-title').append(lymcoin_customizer_vars['theme_name_suffix']);

		// Blur the "load fonts" fields - regenerate options lists in the font-family controls
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', lymcoin_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', lymcoin_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			lymcoin_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) lymcoin_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		lymcoin_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		lymcoin_customizer_refresh_preview(obj);
	});

	// Return value of the control
	function lymcoin_customizer_get_field_value(fld) {
	...
	function lymcoin_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			var fld=null, val='', i;
			var depend = false;
			for (fld in lymcoin_dependencies) {
				if (fld == id) {
			...
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = lymcoin_customizer_get_field_value(fld);
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					ctrl.show().removeClass('lymcoin_options_no_use');
				} else {
					ctrl.hide().addClass('lymcoin_options_no_use');
				}
			}
			...
			if (id == 'color_scheme') {		// Disable color schemes less then main scheme!
				...
				if (fld.length > 0) {
					val = lymcoin_customizer_get_field_value(fld);
					var num = 0;
					for (i in lymcoin_color_schemes) {
						num++;
						if (i == val) break;
					}
					cont.find('.customize-control').each(function() {
	...
	function lymcoin_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in lymcoin_theme_fonts) {
					for (rule in lymcoin_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							lymcoin_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) lymcoin_customizer_update_css();
		}
	...
	// Actions buttons
	function lymcoin_customizer_actions(e) {
	...
	function lymcoin_customizer_update_theme_fonts(opt, rule, value) {
		lymcoin_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function lymcoin_customizer_update_load_fonts() {
        var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
        updateCSS = false;
        for (i=1; i <= lymcoin_customizer_vars['max_load_fonts']; i++) {
	...
	function lymcoin_customizer_update_css() {
		...
		var fonts = lymcoin_customizer_add_theme_fonts(lymcoin_theme_fonts);

		// Make styles and add into css
		css += lymcoin_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in lymcoin_color_schemes) {
			
			var i, colors = [];
			
			// Copy all colors!
			for (i in lymcoin_color_schemes[scheme].colors) {
				colors[i] = lymcoin_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = lymcoin_customizer_add_theme_colors(colors);
            ...
            var tmpl = jQuery( '#tmpl-lymcoin-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function lymcoin_customizer_add_theme_colors(colors) {
		if (lymcoin_additional_colors) {
			var clr = '', v = '';
			for (var k in lymcoin_additional_colors) {
				v = lymcoin_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + lymcoin_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + lymcoin_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + lymcoin_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function lymcoin_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function lymcoin_customizer_prepare_html_value(val) {
