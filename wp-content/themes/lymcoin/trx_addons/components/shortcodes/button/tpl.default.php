	...
	if (!empty($args['image']) || !empty($args['icon'])) {
		?><span class="sc_button_icon"><?php
			if (!empty($args['image'])) {
				...
				?><img class="sc_icon_as_image" src="<?php echo esc_url($args['image']); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			} else if (trx_addons_is_url($args['icon'])) {
				$attr = trx_addons_getimagesize($args['icon']);
				?><img class="sc_icon_as_image" src="<?php echo esc_url($args['icon']); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
