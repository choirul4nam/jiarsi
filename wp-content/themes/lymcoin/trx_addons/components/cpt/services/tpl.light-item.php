...
?>>
	...
	if ($args['featured'] != 'none') {
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			?><img src="<?php echo esc_url($meta['image']); ?>" <?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
			echo !empty($link) 
				? '</a>' 
				: '</span>';
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header"><?php
			$title_tag = $args['featured_position'] == 'top' ? 'h4' : 'h5';
			?>
			<<?php echo esc_attr($title_tag); ?> class="sc_services_item_title<?php if (!$price_showed && !empty($meta['price'])) echo ' with_price'; ?>"><?php
				if (!empty($link)) {
		...
		if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
			...
			if (!empty($link)) {
				?><div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button_default sc_button_size_small', 'sc_services', $args)); ?>"><?php esc_html_e('Learn more', 'lymcoin'); ?></a></div><?php
			}
		}
	?></div>
</div><?php
