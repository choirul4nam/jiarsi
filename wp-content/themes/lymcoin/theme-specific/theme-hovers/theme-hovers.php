...
if (!function_exists('lymcoin_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'lymcoin_hovers_theme_setup3', 3 );
	function lymcoin_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		lymcoin_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'lymcoin'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'lymcoin') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'lymcoin'),
					'slide_left'	=> esc_html__('Slide from Left',	'lymcoin'),
					'slide_right'	=> esc_html__('Slide from Right',	'lymcoin'),
					'slide_top'		=> esc_html__('Slide from Top',		'lymcoin'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'lymcoin'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'lymcoin'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'lymcoin') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'lymcoin'),
					'icon'	=> esc_html__('Icon',	'lymcoin'),
					'icons'	=> esc_html__('Icons',	'lymcoin'),
					'zoom'	=> esc_html__('Zoom',	'lymcoin'),
					'fade'	=> esc_html__('Fade',	'lymcoin'),
					'slide'	=> esc_html__('Slide',	'lymcoin'),
					'pull'	=> esc_html__('Pull',	'lymcoin'),
					'border'=> esc_html__('Border',	'lymcoin')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('lymcoin_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'lymcoin_hovers_theme_setup9', 9 );
	function lymcoin_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',			'lymcoin_hovers_frontend_scripts', 1010 );
		add_filter( 'lymcoin_filter_localize_script','lymcoin_hovers_localize_script' );
		add_filter( 'lymcoin_filter_merge_scripts',	'lymcoin_hovers_merge_scripts' );
		add_filter( 'lymcoin_filter_merge_styles',	'lymcoin_hovers_merge_styles' );
		add_filter( 'lymcoin_filter_merge_styles_responsive', 'lymcoin_hovers_merge_styles_responsive' );
		add_filter( 'lymcoin_filter_get_css', 		'lymcoin_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'lymcoin_hovers_frontend_scripts' ) ) {
	
	function lymcoin_hovers_frontend_scripts() {
		if ( lymcoin_is_on(lymcoin_get_theme_option('debug_mode')) && lymcoin_get_file_dir('theme-specific/theme-hovers/theme-hovers.js')!='' )
			wp_enqueue_script( 'lymcoin-hovers', lymcoin_get_file_url('theme-specific/theme-hovers/theme-hovers.js'), array('jquery'), null, true );
	}
}

// Merge hover effects into single css
if (!function_exists('lymcoin_hovers_merge_styles')) {
	
	function lymcoin_hovers_merge_styles($list) {
...
if (!function_exists('lymcoin_hovers_merge_styles_responsive')) {
	
	function lymcoin_hovers_merge_styles_responsive($list) {
...
if (!function_exists('lymcoin_hovers_localize_script')) {
	
	function lymcoin_hovers_localize_script($arr) {
		$arr['button_hover'] = lymcoin_get_theme_option('button_hover');
		return $arr;
	}
}

// Merge hover effects into single js
if (!function_exists('lymcoin_hovers_merge_scripts')) {
	
	function lymcoin_hovers_merge_scripts($list) {
...
if ( !function_exists('lymcoin_hovers_add_icons') ) {
	function lymcoin_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat'])) {
					lymcoin_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? ($product->is_type( 'variable' ) ? esc_html__('Select options', 'lymcoin') : esc_html__('Buy now', 'lymcoin')) : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'lymcoin');
						else
							esc_html_e('Details', 'lymcoin');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'lymcoin_hovers_get_css' ) ) {
	
	function lymcoin_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
/* Slide */
.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_hover']} !important; }
.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_hover']} !important; }
.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_hover']} !important; }
...
.sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_link']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_top {			background: linear-gradient(to bottom,	{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_link']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['text_link']} 50%, {$colors['text_dark']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_link']} !important; }
...
.sc_button_hover_style_link2.sc_button_hover_slide_bottom  {	color: {$colors['text_dark']} !important; }
...
.sc_button_hover_style_link2.sc_button_hover_slide_top:hover,
.sc_button_hover_style_link2.sc_button_hover_slide_bottom:hover  {	color: {$colors['inverse_text']} !important; }
...
.sc_button_hover_style_dark.sc_button_hover_slide_right:hover *,
.sc_button_hover_style_dark.sc_button_hover_slide_top:hover *,
.sc_button_hover_style_dark.sc_button_hover_slide_bottom:hover * {	color: {$colors['bg_color']} !important; }
