...
if (!function_exists('lymcoin_customizer_get_css')) {

	function lymcoin_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = lymcoin_customizer_add_theme_fonts($fonts);
...
EOT;
			$rez = apply_filters('lymcoin_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(lymcoin_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = lymcoin_get_scheme_colors($scheme);
...
table:not(.tribe-events-calendar) td, table:not(.tribe-events-calendar) th + td, table:not(.tribe-events-calendar) td + td {
...
#trx_addons_login_popup input:focus,
...
#bbpress-forums div.bbp-the-content-wrapper input[type=button]:focus,
...
.lymcoin_tabs .lymcoin_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.lymcoin_tabs .lymcoin_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .lymcoin_tabs .lymcoin_tabs_titles li.ui-state-active a {
...
EOT;
					...
					$rez = apply_filters('lymcoin_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'lymcoin_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>